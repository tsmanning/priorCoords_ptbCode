...
refVels1    = [4];                            % Reference velocities (deg/s)
...
refVels2    = [4];                            % Reference velocities (deg/s)
...
refVels3    = [4];                            % Reference velocities (deg/s)
stairTypes3 = [2 1; 1 2];                     % x up/y down (inds)
...
% Size control judgment 2
stairStruct4 = stairStruct;

contCombs4  = [0.5 0.5];                      % Reference/Test constrasts (%)
sizeCombs4  = [1 0.5;0.5 1];                  % Reference/Test sizes (m)
refPos4     = [1];                            % Ref Near/Far (Screen inds)
testPos4    = [1];                            % Test on same or different screen (1-y/2-n)
refVels4    = [4];                            % Reference velocities (deg/s)
stairTypes4 = [2 1; 1 2];                     % x up/y down (inds)

stairStruct4.contCombs  = contCombs4;
stairStruct4.sizeCombs  = sizeCombs4;
stairStruct4.refPos     = refPos4;
stairStruct4.testPos    = testPos4;
stairStruct4.refVels    = refVels4;
stairStruct4.stairTypes = stairTypes4;
...
switch type
    case 'demo'
        ...
        % Demos
        [ds1,pa1,kb1,tx1,gSCell1] = runExperiment_gabors({subjID,['demoWithin',types{a}],1,perspective},dists,dispID,db,stairDemo1);
        [ds2,pa2,kb2,tx2,gSCell2] = runExperiment_gabors({subjID,['demoBetween',types{a}],2,perspective},dists,dispID,db,stairDemo2);
        
    case 'full'
%         if a == 1              
%             % Within
%             [ds1,pa1,kb1,tx1,gSCell1] = runExperiment_gabors({subjID,['fullWithin',types{a}],1,perspective},dists,dispID,db,stairStruct1);
            
            % Between
            [ds2,pa2,kb2,tx2,gSCell2] = runExperiment_gabors({subjID,['fullBetween',types{a}],2,perspective},dists,dispID,db,stairStruct2);
        
%             % Size control
%             [ds3,pa3,kb3,tx3,gSCell3] = runExperiment_gabors({subjID,['fullWithin',types{a}],3,perspective},dists,dispID,db,stairStruct3);
%         else
%             % Between
%             [ds2,pa2,kb2,tx2,gSCell2] = runExperiment_gabors({subjID,['fullBetween',types{a}],2,perspective},dists,dispID,db,stairStruct2);
%             
...
%         end
    ...
    case 'sizeCont'
        [ds4,pa4,kb4,tx4,gSCell4] = runExperiment_gabors_sizeControl({subjID,['fullSizeCont',types{a}],3,perspective},dists,dispID,db,stairStruct4);
