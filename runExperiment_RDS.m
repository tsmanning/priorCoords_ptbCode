function [ds,pa,kb,tx,gSCell] = runExperiment_RDS(varargin)
...
[pa] = SetupParameters_RDS(pars,ds,autoPars);      
...
[ds,pa,kb,gSCell] = SetupNewTrial_RDS(ds,pa,kb,gSCell,currentScInd);

% Initialize random dot stimulus
[tx] = initializeRDS(pa);
...
while (currentScInd) && ~kb.keyCode(kb.escapeKey)
    ...
    elseif (ds.vbl > pa.trialOnset + pa.fixDuraA) && ...
        ...
        % Draw RDS dot positions for this frame (3 - using PTB built-in shader)
        ...
        if pa.refFirst || pa.trialEpochs == 1
            ...
            % Draw stimulus dots
            Screen('DrawDots',pa.refWind,tx.currentDotPos{pa.refScreen,pa.refPos}',pa.dotSize(pa.refScreen),...
                pa.refLum,[],3);
            ...
            % Draw stimulus dots
            Screen('DrawDots',pa.testWind,tx.currentDotPos{pa.testScreen,pa.testPos}',pa.dotSize(pa.testScreen),...
                pa.testLum,[],3);
        ...
        if pa.trialEpochs == 1
            ...
            Screen('DrawDots',pa.refWind,tx.currentDotPos{pa.refScreen,pa.testPos}',pa.dotSize(pa.refScreen),...
                pa.testLum,[],3);
        ...
        % Get dot positions for the next frame
        [tx,velmatdeg,velmat] = updateRDS(tx,ds,pa);
    ...
    elseif (((ds.vbl > pa.trialOnset + pa.fixDuraA + pa.fixDuraB + pa.trialDuraA) && kb.responseGiven) || ...
        ...
        % Initialize the RDS for the next trial (SHOULD PROB GO IN SETUP
        % NEWTRIAL
        [tx] = initializeRDS(pa);
...
%         Screen('FillRect',pa.trialWind1,[ 0 0 0 1]);
%         Screen('FillRect',pa.trialWind1,[ 0.5 0.5 0.5 1],ds.vignetteRects(pa.trialScrn1,:));
...
%         pa.EOTflag = 1;
    ...
    elseif pa.breakTime
        ...
        percComplete = round(100*sum(currentNumTrials)/(stairPars.maximumtrials*pa.numStaircases));
