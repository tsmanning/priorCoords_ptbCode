function [ds,pa,kb,tx,gSCell] = runExperiment_gabors_sizeControl(varargin)
...
sizeCombs  = stairPars.sizeCombs;   % Reference/Test size (near/far)
...
pa.numStaircases = size(contCombs,1)*size(sizeCombs,1)*numel(refPos)*numel(refVels)*size(stairTypes,1);
                                 
scInds = makeCombos([size(contCombs,1) size(sizeCombs,1) numel(refPos) numel(refVels) size(stairTypes,1)]);
...
for ii = 1:pa.numStaircases
        ...
        gSCell{ii} = set(gSCell{ii},'refSize',sizeCombs(scInds(ii,2),1));
        gSCell{ii} = set(gSCell{ii},'testSize',sizeCombs(scInds(ii,2),2));
        gSCell{ii} = set(gSCell{ii},'refScreen',refPos(scInds(ii,3)));
        gSCell{ii} = set(gSCell{ii},'refVelocity',refVels(scInds(ii,4)));
        gSCell{ii} = set(gSCell{ii},'numUp',stairTypes(scInds(ii,5),1));
        gSCell{ii} = set(gSCell{ii},'numDown',stairTypes(scInds(ii,5),2));    
        gSCell{ii} = set(gSCell{ii},'testScreen',testPos(scInds(ii,6)));
        gSCell{ii} = set(gSCell{ii},'refPosition',randi(2));
        
        minVal = exp( log(refVels(scInds(ii,4))) - 1.8);
        maxVal = min([exp( log(refVels(scInds(ii,4))) + 1.8) 20]);
...
[ds,pa,kb,gSCell] = SetupNewTrial_gabors_sizeControl(ds,pa,kb,gSCell,currentScInd);
...
while (currentScInd) && ~kb.keyCode(kb.escapeKey)
    ...
    % response epoch)
    [pa,kb]     = GetResponse_RDS_sizeControl(pa,ds,kb);
    ...
    elseif (ds.vbl > pa.trialOnset + pa.fixDuraA) && ...
        ...
        if pa.refFirst || pa.trialEpochs == 1
            ...
            % Draw Gabor texture
            Screen('DrawTextures',pa.refWind,tx.gaborTex{pa.refSz},[],squeeze(tx.dstRects(pa.refPos,:,pa.refSz))',...
                              tx.rotAngles(1),[],[],tx.modulateColor,[],...
                              kPsychDontDoRotation,tx.gaborPars(:,pa.refPos,pa.refSz));
            ...
            tx.gaborPars(1,pa.refPos,pa.refSz) = tx.gaborPars(1,pa.refPos,pa.refSz) + pa.thisPhaseOff(1);
            ...
            % Draw Gabor texture
            Screen('DrawTextures',pa.testWind,tx.gaborTex{pa.testSz},[],squeeze(tx.dstRects(pa.testPos,:,pa.testSz))',...
                              tx.rotAngles(1),[],[],tx.modulateColor,[],...
                              kPsychDontDoRotation,tx.gaborPars(:,pa.testPos,pa.testSz));
            ...
            % Advance phases for next frame (deg of cycle) to produce apparent motion
            % [parameters,position,screen/distance]
            tx.gaborPars(1,pa.testPos,pa.testSz) = tx.gaborPars(1,pa.testPos,pa.testSz) + pa.thisPhaseOff(2);
        ...
        if pa.trialEpochs == 1
            ...
            Screen('DrawTextures',pa.refWind,tx.gaborTex{pa.testSz},[],squeeze(tx.dstRects(pa.testPos,:,pa.testSz))',...
                              tx.rotAngles(1),[],[],tx.modulateColor,[],...
                              kPsychDontDoRotation,tx.gaborPars(:,pa.testPos,pa.testSz));
...
%         pa.phases = [pa.phases;[squeeze(tx.gaborPars(1,:,pa.refScreen)) ds.vbl]];
    ...
    elseif (ds.vbl > pa.trialOnset + pa.fixDuraA + pa.fixDuraB + pa.trialDuraA + pa.trialDuraB) && ...
        ...
        DrawFormattedText(pa.refWind,inst{pa.trialEpochs},'center',ds.textCoords(1,2)-100);
